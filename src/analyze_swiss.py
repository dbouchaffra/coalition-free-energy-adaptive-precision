"""
Multi-Agent Credit Assignment & Adaptive Precision Control
Run on any Swiss Roundabout CSV file.
Usage: python analyze_swiss.py <filename.csv>
If no filename provided, default to D1_AM2_F1.csv
"""

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import os
import sys
from scipy.optimize import curve_fit
from sklearn.model_selection import train_test_split

# ==========================================
# Get filename from command line or use default
# ==========================================
if len(sys.argv) > 1:
    csv_file = sys.argv[1]
else:
    csv_file = "D1_AM2_F1.csv"

if not os.path.exists(csv_file):
    print(f"Error: File {csv_file} not found.")
    sys.exit(1)

print(f"Analyzing file: {csv_file}")

# ==========================================
# Load trajectories
# ==========================================
def load_swiss_trajectories(csv_path):
    df = pd.read_csv(csv_path)
    id_col = 'track_id' if 'track_id' in df.columns else 'id'
    x_col = 'x' if 'x' in df.columns else 'lon'
    y_col = 'y' if 'y' in df.columns else 'lat'
    time_col = 'time' if 'time' in df.columns else None
    if time_col:
        df = df.sort_values(time_col)
    trajectories = []
    for agent_id, group in df.groupby(id_col):
        positions = group[[x_col, y_col]].values.astype(np.float32)
        if len(positions) >= 10:
            trajectories.append({"agent_id": agent_id, "positions": positions})
    print(f"Loaded {len(trajectories)} agents.")
    return trajectories

trajectories = load_swiss_trajectories(csv_file)
if len(trajectories) == 0:
    print("No trajectories found.")
    sys.exit(1)

# ==========================================
# Prepare sequences (input_len=10, output_len=5)
# ==========================================
def split_sequence(pos_seq, input_len=10, output_len=5):
    X, Y = [], []
    for i in range(len(pos_seq) - input_len - output_len + 1):
        X.append(pos_seq[i:i+input_len])
        Y.append(pos_seq[i+input_len:i+input_len+output_len])
    return np.array(X), np.array(Y)

all_X, all_Y = [], []
for traj in trajectories:
    X, Y = split_sequence(traj["positions"])
    if len(X) > 0:
        all_X.append(X)
        all_Y.append(Y)
X_all = np.concatenate(all_X, axis=0)
Y_all = np.concatenate(all_Y, axis=0)
print(f"Total samples: {X_all.shape[0]}")

# Split into train and validation (80/20)
X_train, X_val, Y_train, Y_val = train_test_split(X_all, Y_all, test_size=0.2, random_state=42)

# ==========================================
# Linear predictor and Shapley
# ==========================================
class LinearPredictor:
    def __init__(self, input_len, output_len):
        self.input_len = input_len
        self.output_len = output_len
        self.W = None
    def fit(self, X, Y):
        X_flat = X.reshape(X.shape[0], -1)
        Y_flat = Y.reshape(Y.shape[0], -1)
        self.W = np.linalg.lstsq(X_flat, Y_flat, rcond=None)[0]
    def predict(self, X):
        X_flat = X.reshape(X.shape[0], -1)
        Y_pred = X_flat @ self.W
        return Y_pred.reshape(X.shape[0], self.output_len, 2)
    def mae(self, X, Y):
        Y_pred = self.predict(X)
        return np.mean(np.sqrt(np.sum((Y_pred - Y)**2, axis=2)))  # RMSE per point, but we call it MAE for brevity

def shapley_value(model, X, Y, n_permutations=50):
    input_len = X.shape[1]
    n_features = input_len
    contribs = []
    for _ in range(n_permutations):
        perm = np.random.permutation(n_features)
        X_zero = np.zeros_like(X)
        baseline_err = model.mae(X_zero, Y)
        err_prev = baseline_err
        for j in range(n_features):
            mask = np.zeros(n_features, dtype=bool)
            mask[perm[:j+1]] = True
            X_masked = X.copy()
            X_masked[:, ~mask, :] = 0.0
            err = model.mae(X_masked, Y)
            contribs.append(err_prev - err)
            err_prev = err
    return np.mean(contribs)

def add_noise(X, beta):
    sigma = 1.0 / np.sqrt(max(beta, 1e-6))
    noise = np.random.normal(0, sigma, X.shape)
    return X + noise

# ==========================================
# Inverted-U validation
# ==========================================
betas = np.linspace(0.5, 5.0, 10)
shap_vals = []
for beta in betas:
    print(f"β = {beta:.2f}")
    X_noisy = add_noise(X_train, beta)
    model = LinearPredictor(10, 5)
    model.fit(X_noisy, Y_train)
    shap = shapley_value(model, X_noisy, Y_train, n_permutations=20)
    shap_vals.append(shap)

def quadratic(x, a, b, c):
    return a*x**2 + b*x + c
popt, _ = curve_fit(quadratic, betas, shap_vals)
beta_opt = -popt[1]/(2*popt[0]) if popt[0] < 0 else betas[np.argmax(shap_vals)]

# Save inverted-U figure with custom name based on input file
base_name = os.path.splitext(os.path.basename(csv_file))[0]
fig1_name = f"invertedU_{base_name}.png"
plt.figure(figsize=(6,4))
plt.plot(betas, shap_vals, 'bo-', label='Estimated Shapley')
plt.plot(betas, quadratic(betas, *popt), 'r--', label=f'Quadratic fit (peak β≈{beta_opt:.2f})')
plt.xlabel('Sensory precision β')
plt.ylabel('Credit assignment (Shapley value)')
plt.title(f'Inverted‑U Relationship ({base_name})')
plt.legend()
plt.grid(True)
plt.savefig(fig1_name, dpi=150)
plt.show()
print(f"Inverted‑U peak at β = {beta_opt:.2f}. Figure saved as {fig1_name}")

# ==========================================
# Adaptive Precision Control (APC)
# ==========================================
class APC:
    def __init__(self, initial_beta=1.5, learning_rate=0.05, window=10):
        self.beta = initial_beta
        self.eta = learning_rate
        self.window = window
        self.history = []
    def update(self, credit):
        self.history.append((self.beta, credit))
        if len(self.history) < 3:
            return
        if len(self.history) > self.window:
            self.history.pop(0)
        betas_win = np.array([h[0] for h in self.history])
        creds_win = np.array([h[1] for h in self.history])
        try:
            a, b, c = np.polyfit(betas_win, creds_win, 2)
            if a < 0:
                grad = 2*a*self.beta + b
                self.beta += self.eta * grad
                self.beta = np.clip(self.beta, 0.2, 5.0)
            else:
                self.beta += 0.1 * (np.random.rand() - 0.5)
                self.beta = np.clip(self.beta, 0.2, 5.0)
        except:
            pass

print("\nRunning Adaptive Precision Control (APC)...")
apc = APC(initial_beta=2.0)
epochs = 50
credits_apc = []
apc_mae = None
for epoch in range(epochs):
    X_noisy = add_noise(X_train, apc.beta)
    model = LinearPredictor(10, 5)
    model.fit(X_noisy, Y_train)
    shap = shapley_value(model, X_noisy, Y_train, n_permutations=10)
    credits_apc.append(shap)
    apc.update(shap)
    if epoch % 10 == 0:
        print(f"Epoch {epoch}: β={apc.beta:.3f}, credit={shap:.4f}")
# After training, compute MAE on validation set for the final model
X_val_noisy = add_noise(X_val, apc.beta)
apc_mae = model.mae(X_val_noisy, Y_val)

# Fixed β baselines
betas_fixed = [0.5, 2.0, 5.0]
credits_fixed = {}
fixed_maes = {}
for beta_fixed in betas_fixed:
    credits = []
    for epoch in range(epochs):
        X_noisy = add_noise(X_train, beta_fixed)
        model = LinearPredictor(10, 5)
        model.fit(X_noisy, Y_train)
        shap = shapley_value(model, X_noisy, Y_train, n_permutations=10)
        credits.append(shap)
    credits_fixed[beta_fixed] = credits
    # Compute MAE on validation set for this fixed β
    X_val_noisy = add_noise(X_val, beta_fixed)
    fixed_maes[beta_fixed] = model.mae(X_val_noisy, Y_val)

# Find best fixed β (the one with lowest validation MAE)
best_fixed_beta = min(fixed_maes, key=fixed_maes.get)
best_fixed_mae = fixed_maes[best_fixed_beta]
reduction = (best_fixed_mae - apc_mae) / best_fixed_mae * 100

fig2_name = f"apc_{base_name}.png"
plt.figure(figsize=(8,5))
plt.plot(credits_apc, label='APC (adaptive β)', linewidth=2)
for beta_fixed, creds in credits_fixed.items():
    plt.plot(creds, '--', label=f'Fixed β={beta_fixed}', alpha=0.7)
plt.xlabel('Epoch')
plt.ylabel('Credit assignment (Shapley value)')
plt.title(f'APC vs Fixed Precision ({base_name})')
plt.legend()
plt.grid(True)
plt.savefig(fig2_name, dpi=150)
plt.show()

print("\n✅ Experiments completed.")
print(f"Final β after APC: {apc.beta:.3f}")
print(f"Best fixed β (β={best_fixed_beta}) MAE: {best_fixed_mae:.4f}")
print(f"APC MAE: {apc_mae:.4f}")
print(f"Relative reduction: {reduction:.1f}%")
print(f"Figures saved: {fig1_name}, {fig2_name}")
